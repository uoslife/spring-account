...
class SmsService(
    ...
    fun sendMessage(mobile: String, title: String, message: String): Boolean {
        if (env == "prod") {
    ...
    private fun sendWithDevSend(mobile: String, subtitle: String, message: String): Boolean {
        val title = "💬 $mobile"
        logger.info(title + subtitle + message)
        return true
    }

    private fun sendWithDirectSend(mobile: String, title: String, message: String): Boolean {
        val smsRequest =
            SmsParamDto.SmsRequest(
                sender = sender,
